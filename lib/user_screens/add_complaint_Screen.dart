...
class AddComplaintScreenV extends StatefulWidget {
  const AddComplaintScreenV({Key? key}) : super(key: key);

  @override
  State<AddComplaintScreenV> createState() => _AddComplaintScreenVState();
}

class _AddComplaintScreenVState extends State<AddComplaintScreenV> {
  bool loading = false;
  ...
  Future<void> getPersonImage() async {
    ...
    setState(() {
      _personImage = pickedFile != null ? File(pickedFile.path) : null;
    });
  }
                  ...
                  if (_formKey.currentState!.validate()) {
                    ...
                    try {
                      ...
                      await databaseRef.doc(reportId).set({
                        'reportId': reportId,
                        'reportType':
                            reportType == ReportType.lost ? 'lost' : 'found',
